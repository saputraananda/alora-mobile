...
function LiveTimestamp({ align = 'left' }) {
...
export default function MobileCameraCapture({
  ...
  const capture = () => {
    ...
    if (staticLocationLabel) {
      locationLine = staticLocationLabel;
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/70 px-3 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 sm:p-4">
      <div className="w-full max-w-[430px] max-h-[calc(100dvh-24px)] bg-white rounded-[18px] overflow-hidden shadow-[0_12px_60px_rgba(0,0,0,.35)] flex flex-col">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div className="text-[13px] font-extrabold text-navy-950 truncate pr-3">{title}</div>
          <button
            type="button"
            onClick={handleClose}
        ...
        <div className="p-3 sm:p-4 overflow-y-auto">
          ...
          <div className="rounded-[16px] overflow-hidden bg-black relative aspect-[3/4] sm:aspect-[4/5]">
            {camError ? (
              ...
              <>
                ...
                />
                {ready ? <LiveTimestamp align="left" /> : null}
                <button
                  type="button"
                  onClick={flipCamera}
                  disabled={Boolean(camError)}
                  className="absolute top-2 right-2 w-9 h-9 rounded-full bg-black/50 grid place-items-center text-white disabled:opacity-30"
                ...
                {includeLocation ? (
                  <div className="absolute top-12 left-3 right-3 text-[10.5px] font-semibold">
                    <div
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sticky bottom-0 bg-white pt-1.5">
            ...
            <button
              ...
              className="h-[40px] rounded-[12px] bg-navy-950 text-white text-[12px] font-extrabold disabled:opacity-50"
            >
              {!ready
                ? 'Menyiapkan...'
                : includeLocation && gpsStatus !== 'ready'
