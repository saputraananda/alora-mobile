...
export default function Home() {
  ...
  const handlePageRefresh = useCallback(async () => {
    await loadBroadcasts({ retries: 1, delayMs: 0, silent: true });
    try {
      const roleRes = await homeApi.get('/auth/leader-role');
      setLeaderRole(roleRes.data?.data?.role || null);
    } catch {
      setLeaderRole(null);
    }
    try {
      const summaryRes = await homeApi.get('/approvals/summary');
      setApprovalAccess(Boolean(summaryRes.data?.can_access));
      setApprovalTotal(Number(summaryRes.data?.total) || 0);
    } catch {
      setApprovalAccess(false);
      setApprovalTotal(0);
    }
      ...
      <header className="relative pt-5 pb-6 px-6 bg-[#050B14] rounded-b-[36px] overflow-hidden shadow-xl text-white">
        ...
        <div className="relative z-20 flex items-center justify-between gap-3 mb-1">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <PageHeaderRefreshButton
              onRefresh={handlePageRefresh}
              className="!w-10 !h-10 !rounded-full border border-white/20 bg-white/10 hover:bg-white/20 transition active:scale-95 backdrop-blur-md"
            />
            <button
