...
import { User, Lock, Eye, EyeOff, LogIn, AlertCircle, ScanFace } from 'lucide-react';

import aloraMobileLogo from '../../../assets/images/aloramobile-white.webp';
import Modal from '../../../components/Modal.jsx';
import FaceScanModal from '../components/FaceScanModal.jsx';
import { useDocumentTitle } from '../../../hooks/useDocumentTitle.js';
import { getAuthClient, setAuthSession } from '../../../utils/authSession.js';
...
export default function Login({ onLoginSuccess }) {
  ...
  const handleFaceLoginComplete = async (descriptors) => {
    setFaceLoading(true);

    const verifyRes = await axios.post(
      '/api/auth/face/login',
      { descriptors, client: getAuthClient() },
    ...
    throw new Error(verifyRes.data?.message || 'Login wajah gagal.');
  };
  ...
  return (
    ...
    <div className="flex flex-col min-h-screen bg-[#050B14] relative overflow-hidden justify-between w-full">
      ...
      <div className="relative z-30 bg-white rounded-t-[36px] shadow-[0_-12px_40px_rgba(0,0,0,0.35)] px-6 pt-6 pb-6 border-t border-slate-100 flex flex-col animate-fade-in mt-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            ...
            className="w-full py-3.5 px-4 rounded-[16px] bg-white border-2 border-navy-950 text-navy-950 font-extrabold text-xs hover:bg-slate-50 active:scale-[0.99] transition flex items-center justify-center gap-2 disabled:opacity-50"
          ...
          <p className="text-[10px] text-slate-400 text-center leading-relaxed">
            Scan wajah otomatis — posisikan wajah di kamera. Daftar wajah di Profil setelah login pertama.
          </p>
      ...
      <FaceScanModal
        open={faceScanOpen}
        mode="autoLogin"
        title="Login dengan Wajah"
        onComplete={handleFaceLoginComplete}
        onClose={() => !faceLoading && setFaceScanOpen(false)}
        busy={faceLoading}
